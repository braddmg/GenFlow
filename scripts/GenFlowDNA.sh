...
Help() {
    ...
    echo
    echo "Syntax: GenFlow [-f|-h|-g|-t|-G|-F|-DNA]"
    echo "Options:"
    echo "-g     A txt file containing all accession numbers of reference genomes (default: genomes.txt)"
    echo "-h     Print this Help."
    echo "-f     FASTA file(s). If you have more than one, provide file names separated by coma:"
    ...
    echo "-DNA   Use this option to work with DNA sequences instead of protein sequences."
}
...
while getopts ":hf:g:t:G:F:DNA" option; do
    case $option in
        h) # Display Help
            Help
            exit;;
...
if [ "$DNA_mode" = true ]; then
    # If -DNA option is set, modify the anvi-get-sequences-for-gene-clusters command
...
if [ "$DNA_mode" = true ]; then
    mafft --retree 1 --thread "$threads" --maxiterate 0 dna-sequences.fasta > dna-sequences-aligned.fasta
    FastTree -gtr -nt < dna-sequences-aligned.fasta > ../results/phylogenomic-tree.txt >> ../logs/fasttree.log 2>&1
else
    mafft --retree 1 --thread "$threads" --maxiterate 0 proteins-sequences.fasta > proteins-sequences-aligned.fasta
    anvi-gen-phylogenomic-tree -f proteins-sequences-aligned.fasta \
                               -o ../results/phylogenomic-tree.txt >> ../logs/fasttree.log 2>&1
fi
...
average_nucleotide_identity.py -i fa \
                               ...
                               --labels labels.txt \
                               --classes classes.txt \
                               -g --gmethod seaborn --gformat svg -v -l ba_ANI.log >> ../logs/pyANI.log 2>&1
